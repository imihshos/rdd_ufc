---
title: "DBA4712 Project Code Final"
author: "Soh Si Ying"
date: "2022-11-15"
...
We first round all the odds to the nearest 10 so that there are no odds with only 1 data point.
...
Based on the results, the betting favourites and underdogs are different in the covariate values for R_losses, loss_dif, lose_streak_dif, win_streak_dif,total_round_dif, longest_win_streak_dif, therefore there is no like vs like comparison
between the two groups. To handle the covariate imbalance, OLS was used for regression adjustments.
```{r}
...
longest_win_streak_dif) were set as the independent variables. 
```{r}
